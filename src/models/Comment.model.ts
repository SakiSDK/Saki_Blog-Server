...
export interface CommentReply extends Omit<CommentAttributes, 'content' | 'post_id' | 'user_id' | 'user_ip' | 'updated_at' | 'status'> {
    content: string; // 格式化后的内容
    user?: Pick<User, 'nickname' | 'avatar' | 'short_id'>;
}

// 定义「主评论」的类型（根评论，replies平级包含所有回复）
export interface MainComment extends Omit<CommentAttributes, 'content' | 'parent_id' | 'post_id' | 'user_id' | 'user_ip' | 'updated_at' | 'status'> {
    content: string; // 格式化后的内容
    user?: Pick<User, 'nickname' | 'avatar' | 'short_id'>;
...
export interface CommentAttributes {
    ...
    is_author: boolean;     // 是否是
    user_device?: string;   // 用户设备
    user_browser?: string;    // 用户浏览器信息
    user_region?: string; // 用户所在地
    user_ip?: string;
    created_at: Date;       
    updated_at: Date;
}
...
interface CommentCreationAttributes
    extends Optional<CommentAttributes,
        'id' | 'created_at' | 'updated_at' | 'status' | 'parent_id' |
        'is_author' | "user_device" | "user_browser" | "user_region" |
        "user_ip"
    > { }
...
export class Comment extends Model<CommentAttributes, CommentCreationAttributes> implements CommentAttributes { 
    ...
    public user_device?: string;
    public user_browser?: string;
    ...
    public static async getMainCommentsWithFlatReplies(
        ...
        const whereCondition: any = { post_id: postId };
            ...
            include: [
                {
                    model: User,
                    as: 'user',
                    attributes: ['nickname', 'avatar', 'short_id'], // 只返回必要字段
                    required: false,
                },
                // 新增 Post 关联，获取 short_id
                {
                    model: Post,
                    as: 'post',
                    attributes: [['short_id', 'post_short_id']], // 只返回必要字段
                    required: false,
                }
            ],
            order: [['created_at', 'DESC']],
            attributes: { exclude: ['post_id', 'user_id', 'status', 'user_ip', 'updated_at', 'status'] }, // 排除不必要的字段
        });
        console.log(allComments)

        // 分离主评论和回复
        const mainCommentsRaw = allComments.filter(comment => !comment.parent_id) // 主评论：parent_id为null
        const allRepliesRaw = allComments.filter(comment => comment.parent_id); // 所有回复：parent_id不为null
            ...
            const getRepliesForMainComment = (mainCommentId: number): CommentReply[] => {
                ...
                const findReplies = (parentIds: number[]): CommentReply[] => {
                    const replies = allRepliesRaw
                        .filter(reply => parentIds.includes(reply.parent_id!)) // 父ID在目标列表中
                        .map(reply => {
                ...
                // 1. 收集所有回复 2. 统一按 created_at 排序（核心步骤）
                const allReplies = findReplies([mainCommentId]);
                return allReplies.sort((a, b) => {
                    // 兼容 created_at 是字符串/Date/时间戳的情况，转成时间戳比较
    ...
    },
    {
        ...
        hooks: {
            beforeValidate: async (comment: Comment) => {
                // 自动检查是否是作者回复
                if (!comment.is_author && comment.post_id && comment.user_id) {
                    try {
                        const post = await Post.findByPk(comment.post_id);
                        if (post && comment.user_id === post.author_id) {
                            comment.is_author = true;
                        }
                    } catch (error) {
                        console.error(`[Comment Hook] 标记作者回复失败：post_id=${comment.post_id}, user_id=${comment.user_id}`, error);
                    }
                }
            },
